...
EmployeeRoleView.prototype = {	
	drawSettingForm : function(roleList, employee) {
		var $this = this;
		var roleLength = roleList.length;
		var userRoleLength = employee.roles.length;
		var checkMap = [];
		for(var i=0; i<userRoleLength; i++) {
			checkMap[employee.roles[i].roleId] = true;
		}
		...
		savingButton.click(function(event) {
			...
			$this._savingAction(employee.warehouseEmployeeId, roles, function(){
